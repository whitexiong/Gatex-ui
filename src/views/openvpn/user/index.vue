<template>
  <div>
    ...
    <!-- 用户表格区域 -->
    <el-table :data="Users" row-key="id" style="width: 1980px; height: 1000px" border>
      <el-table-column label="客户端ID" prop="id"></el-table-column>
      <el-table-column label="通用名称" prop="common_name"></el-table-column>
      <el-table-column label="实际地址" prop="real_address"></el-table-column>
      <el-table-column label="接收的字节" prop="bytes_received"></el-table-column>
      <el-table-column label="发送的字节" prop="bytes_sent"></el-table-column>
      <el-table-column label="连接时间" prop="connected_since"></el-table-column>
...
import {getList as getUserList, add, update, detail, deletedById} from '@/services/openvpnUserService';
    ...
    const beforeAvatarUpload = async (rawFile) => {
      ...
      try {
        ...
        if (response && response.data && response.data.avatar_url) {
          User.value.AvatarUrl = "http://10.8.0.6:8051/" + response.data.url;
        }
      } catch (error) {
